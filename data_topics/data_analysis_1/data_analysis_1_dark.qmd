...
This workshop reviews the basics of importing, examining, and manipulating data ("data wrangling") in R, with some brief comparisons to the Python approach to accomplishing the same tasks.
...
First we use _pivot_longer_ to create long format data. [Long vs. Wide data explained](https://www.thedataschool.com.au/mipadmin/the-shape-of-data-long-vs-wide/). Then we filter the data for a recent year (2024 has relatively complete data). The _pivot_wider_ to restore the data to wide format.
...
In much older R code, you will commonly see the "%>%" characters used to pass commands from one line to the next. This is called the "pipe" and is functionality introduced by the _magrittr_ package. Since this was used so much, it has been integrated into the _tidyverse_ without the need for an additional package. The "|>" character combination is the newer way of expressing this (although the older way will still work). When you see this, you can think of it as "and then" we pass to the next line. So "gender_data2024 |> pivot_wider" means take the gender_data2024 data "and then" use the pivot_wider command on it. This kind of syntax is very human-readable and easy-to-understand for steps in data wrangling.
...
# Using Python in RStudio, regression example
...
We will give a quick look at regression in Python, via the [scikit-learn package](https://scikit-learn.org/stable/) which provides Linear Regression and many other machine learning techniques. The [statsmodels](https://www.statsmodels.org/) package provides detailed regression output.
